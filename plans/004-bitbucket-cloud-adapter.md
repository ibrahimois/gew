...
- **Execution result**: Completed in clone/pull-only mode. Recursive pinned
  Source API walking and streamed multipart commit requests are fixture-tested,
  but no Bitbucket Cloud sandbox credentials were available for the mandatory
  stale-parent/lost-response gate, so `Push` remains false in the registry.
