...
func TestBitbucketResolveRepositoryWithBasicAuth(t *testing.T) {
	...
	defer server.Close()
	forge, err := newBitbucketForgeWithAPI(profile{Provider: ForgeBitbucket, URL: server.URL, Token: "secret", AuthKind: AuthBasic, Username: "user@example.com"}, server.URL)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBitbucketTreeWalkPinsCommitAndFollowsPagination(t *testing.T) {
	...
	files, err := forge.Tree(context.Background(), RepositoryRef{Forge: ForgeBitbucket, Namespace: "ws", Name: "repo"}, "commit")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBitbucketMultipartCommitEncoding(t *testing.T) {
	...
	result, err := forge.applyCommitUnchecked(context.Background(), ApplyCommitRequest{
		Repository: RepositoryRef{Forge: ForgeBitbucket, Namespace: "ws", Name: "repo"}, Branch: "main", NewBranch: "feature", ExpectedHead: "base", Message: "message",
		Changes: []RemoteChange{{Operation: "create", Path: "dir/new.bin", Content: []byte{'a', 0, 'b'}}, {Operation: "delete", Path: "old.txt"}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBitbucketPushCapabilityRemainsGated(t *testing.T) {
	forge, _ := newBitbucketForgeWithAPI(profile{Provider: ForgeBitbucket, URL: "https://bitbucket.org", Token: "secret", AuthKind: AuthBearer}, "https://api.bitbucket.org/2.0")
