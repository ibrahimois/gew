...
- **Status**: DONE (offline conformance; no Azure credentials were available for the opt-in live suite)
...
- [x] Azure and forge-contract tests pass with `-race`.
- [x] Full, shuffled, vet, and earlier provider tests pass.
- [x] Stable project/repository IDs survive rename fixtures.
- [x] Snapshots are pinned to one immutable commit.
- [x] One local queued commit produces at most one Azure commit.
- [x] Every existing-branch push carries the exact expected `oldObjectId`.
- [x] A stale head never overwrites remote content or silently rebases.
- [x] Ambiguous results reconcile without duplicate commits.
- [x] Entra/PAT credentials never enter `.gew/` or diagnostics.
- [x] README states Azure DevOps Services scope accurately.
- [x] `docs/plans/README.md` status row is updated.

Execution note: exact request fixtures cover existing refs, documented
all-zero initial refs, branch creation from an expected base, binary/add/edit/
delete batches, 409 conflicts, stable IDs, immutable reads, and cross-origin
redirect isolation. No `GEW_TEST_AZURE_*` credentials or authenticated Azure
CLI session were present, so the optional live Services suite was not run.
