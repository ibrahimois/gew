...
func newGitHubForge(p profile) (*githubForge, error) {
	...
	return &githubForge{server: server, apiBase: apiBase, requester: newHTTPRequester(p, apiBase, auth, headers)}, nil
}
...
func (g *githubForge) ResolveRepository(ctx context.Context, value string) (RepositoryRef, RepositoryInfo, error) {
	owner, name, err := parseGitHubRepository(g.server, value)
	if err != nil {
		return RepositoryRef{}, RepositoryInfo{}, err
	}
	initial := RepositoryRef{Forge: ForgeGitHub, Server: g.server, Namespace: owner, Name: name}
	...
	if response.Owner.Login == "" || response.Name == "" {
		return RepositoryRef{}, RepositoryInfo{}, errors.New("github returned incomplete repository identity")
	}
	...
	ref := RepositoryRef{
		Forge: ForgeGitHub, Server: g.server, Namespace: response.Owner.Login,
		Name: response.Name, RemoteID: strconv.FormatInt(response.ID, 10), Canonical: canonical,
	}
	return ref, RepositoryInfo{DefaultBranch: response.DefaultBranch, Empty: response.PushedAt == nil}, nil
}

func (g *githubForge) Head(ctx context.Context, ref RepositoryRef, branch string) (string, error) {
...
func (g *githubForge) Tree(ctx context.Context, ref RepositoryRef, commit string) (map[string]RemoteFile, error) {
...
func githubRemoteFiles(entries []githubTreeEntry) (map[string]RemoteFile, error) {
	...
	for _, entry := range entries {
		...
		result[cleaned] = RemoteFile{BlobID: entry.SHA, Mode: uint32(mode), Size: entry.Size}
	}
	return result, nil
}

func (g *githubForge) walkTree(ctx context.Context, ref RepositoryRef, rootTree string) (map[string]RemoteFile, error) {
	...
	for len(pending) > 0 {
		...
		for _, entry := range response.Tree {
			...
			cleaned, err := validateRemotePath(combined)
			if err != nil {
				return nil, err
			}
			switch entry.Type {
...
func (g *githubForge) Blob(ctx context.Context, ref RepositoryRef, file RemoteFile) ([]byte, error) {
...
func (g *githubForge) Snapshot(ctx context.Context, ref RepositoryRef, revision string) ([]byte, error) {
	endpoint := githubRepoAPIPath(ref) + "/zipball/" + url.PathEscape(revision)
	return g.requester.download(ctx, endpoint)
}

func (g *githubForge) CommitDetails(ctx context.Context, ref RepositoryRef, commit string) (RemoteCommit, error) {
	result := RemoteCommit{}
	for pageNumber := 1; ; pageNumber++ {
		...
		for _, file := range response.Files {
			cleaned, err := validateRemotePath(file.Filename)
			if err != nil {
				return RemoteCommit{}, err
			}
...
func (g *githubForge) ApplyCommit(ctx context.Context, request ApplyCommitRequest) (ApplyCommitResult, error) {
	if request.ExpectedHead == "" {
		return ApplyCommitResult{}, fmt.Errorf("github cannot create the first branch in an empty repository through the REST Git database API: %w", ErrUnsupported)
	}
	...
	if base.Tree.SHA == "" {
		return ApplyCommitResult{}, errors.New("github base commit has no tree")
	}
	...
	for _, change := range request.Changes {
		...
		case "create", "update":
			...
			if blob.SHA == "" {
				return ApplyCommitResult{}, errors.New("github returned an empty blob ID")
			}
			item.SHA = &blob.SHA
		default:
	...
	if tree.SHA == "" {
		return ApplyCommitResult{}, errors.New("github returned an empty tree ID")
	}
	...
	if created.SHA == "" {
		return ApplyCommitResult{}, errors.New("github returned an empty commit ID")
	}
	...
	if request.NewBranch != "" {
		...
		if response.Object.SHA != created.SHA {
			return ApplyCommitResult{}, errors.New("github created branch points to an unexpected commit")
		}
	} else {
		...
		if response.Object.SHA != created.SHA {
			return ApplyCommitResult{}, errors.New("github updated branch points to an unexpected commit")
		}
	}
	return ApplyCommitResult{CommitID: created.SHA, ParentIDs: []string{request.ExpectedHead}, ConditionalRef: true}, nil
}

func (g *githubForge) gitCommit(ctx context.Context, ref RepositoryRef, commit string) (githubGitCommit, error) {
...
func githubRepoAPIPath(ref RepositoryRef) string {
