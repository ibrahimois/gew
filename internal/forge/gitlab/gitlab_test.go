...
func TestGitLabForgeContract(t *testing.T) {
	forge, err := New(forgecore.Config{Provider: forgecore.ForgeGitLab, URL: "https://gitlab.com", Token: "token", AuthKind: forgecore.AuthBearer})
	if err != nil {
		t.Fatal(err)
	}
	forgetest.RunBaseContract(t, forge, forgecore.ForgeGitLab, true, true, false)
}
...
func TestGitLabResolveNestedProjectAndPrivateToken(t *testing.T) {
	...
	defer server.Close()
	forge, err := New(forgecore.Config{Provider: forgecore.ForgeGitLab, URL: server.URL, Token: "secret", AuthKind: forgecore.AuthPrivate})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGitLabApplyCommitBuildsLockedBatch(t *testing.T) {
	...
	defer server.Close()
	forge, _ := New(forgecore.Config{Provider: forgecore.ForgeGitLab, URL: server.URL, Token: "secret", AuthKind: forgecore.AuthBearer})
	result, err := forge.applyCommitUnchecked(context.Background(), forgecore.ApplyCommitRequest{
		Repository: forgecore.RepositoryRef{Forge: forgecore.ForgeGitLab, RemoteID: "123"}, Branch: "main", NewBranch: "feature", ExpectedHead: "base", Message: "message",
...
func TestGitLabTreePagination(t *testing.T) {
	...
	defer server.Close()
	forge, _ := New(forgecore.Config{Provider: forgecore.ForgeGitLab, URL: server.URL, Token: "secret", AuthKind: forgecore.AuthBearer})
	files, err := forge.Tree(context.Background(), forgecore.RepositoryRef{Forge: forgecore.ForgeGitLab, RemoteID: "1"}, "commit")
	if err != nil {
		t.Fatal(err)
	}
