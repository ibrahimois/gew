...
var (
	_ forge.Forge             = (*gitLabForge)(nil)
	_ forge.ForgeSnapshotter  = (*gitLabForge)(nil)
	_ forge.ForgeCommitWriter = (*gitLabForge)(nil)
)
...
func New(p forge.Config) (*gitLabForge, error) {
	server, err := forge.NormalizeServerURL(p.URL)
	if err != nil {
		return nil, err
	}
	p.Provider = forge.ForgeGitLab
	if p.AuthKind != forge.AuthBearer && p.AuthKind != forge.AuthPrivate {
	...
	auth := func(request *http.Request) {
		if p.AuthKind == forge.AuthPrivate {
...
func (g *gitLabForge) ResolveRepository(ctx context.Context, value string) (forge.RepositoryRef, forge.RepositoryInfo, error) {
	...
	if project.ID == 0 || project.Path == "" || project.PathWithNamespace == "" {
		return forge.RepositoryRef{}, forge.RepositoryInfo{}, errors.New("gitlab returned incomplete project identity")
	}
	...
	ref := forge.RepositoryRef{
		Forge: forge.ForgeGitLab, Server: g.server, Namespace: namespace, Name: project.Path,
		RemoteID: strconv.FormatInt(project.ID, 10), Canonical: project.PathWithNamespace,
	}
	return ref, forge.RepositoryInfo{DefaultBranch: project.DefaultBranch, Empty: project.EmptyRepo}, nil
}

func (g *gitLabForge) Head(ctx context.Context, ref forge.RepositoryRef, branch string) (string, error) {
	var response struct {
...
func (g *gitLabForge) Tree(ctx context.Context, ref forge.RepositoryRef, commit string) (map[string]forge.RemoteFile, error) {
	...
	for pageNumber := 1; ; pageNumber++ {
		...
		for _, entry := range entries {
			...
			result[cleaned] = forge.RemoteFile{BlobID: entry.ID, Mode: uint32(mode)}
		}
		if len(entries) < 100 {
			break
		}
...
func (g *gitLabForge) Blob(ctx context.Context, ref forge.RepositoryRef, file forge.RemoteFile) ([]byte, error) {
...
func (g *gitLabForge) CommitDetails(ctx context.Context, ref forge.RepositoryRef, commit string) (forge.RemoteCommit, error) {
	...
	result := forge.RemoteCommit{ID: response.ID, Message: response.Message, ParentIDs: append([]string(nil), response.ParentIDs...)}
	for pageNumber := 1; ; pageNumber++ {
...
func (g *gitLabForge) ApplyCommit(ctx context.Context, request forge.ApplyCommitRequest) (forge.ApplyCommitResult, error) {
	if !gitLabPushVerified {
		return forge.ApplyCommitResult{}, fmt.Errorf("gitlab push is disabled until live same-file concurrency tests pass: %w", forge.ErrUnsupported)
	}
	return g.applyCommitUnchecked(ctx, request)
}

func (g *gitLabForge) applyCommitUnchecked(ctx context.Context, request forge.ApplyCommitRequest) (forge.ApplyCommitResult, error) {
	if request.ExpectedHead == "" {
		return forge.ApplyCommitResult{}, fmt.Errorf("gitlab empty-repository push is not verified: %w", forge.ErrUnsupported)
	}
	...
	for _, change := range request.Changes {
		...
		case "update", "delete":
			...
			if metadata.LastCommitID == "" {
				return forge.ApplyCommitResult{}, fmt.Errorf("gitlab returned no last_commit_id for %s", change.Path)
			}
	...
	if response.ID == "" {
		return forge.ApplyCommitResult{}, errors.New("gitlab returned an empty commit ID")
	}
	return forge.ApplyCommitResult{CommitID: response.ID, ParentIDs: response.ParentIDs, ConditionalRef: false}, nil
}

func (g *gitLabForge) fileMetadata(ctx context.Context, ref forge.RepositoryRef, filePath, commit string) (gitLabFileMetadata, error) {
...
func gitLabProjectAPIPath(ref forge.RepositoryRef) string {
