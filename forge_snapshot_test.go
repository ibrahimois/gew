...
func TestForgeSnapshotFallbackContract(t *testing.T) {
	...
	first, err := forgeSnapshot(context.Background(), remote, RepositoryRef{Name: "demo"}, "revision")
	if err != nil {
		t.Fatal(err)
	}
	second, err := forgeSnapshot(context.Background(), remote, RepositoryRef{Name: "demo"}, "revision")
	if err != nil {
		t.Fatal(err)
	}
...
func TestForgeSnapshotFallbackRejectsUnsafeObjectsFailuresAndSize(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if _, err := forgeSnapshot(context.Background(), test.remote, RepositoryRef{Name: "demo"}, "revision"); err == nil {
