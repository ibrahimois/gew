...
func TestAzureResolveRepositoryUsesPATAndStableIDs(t *testing.T) {
	...
	defer server.Close()
	forge, err := newAzureForgeWithAPI(profile{Provider: ForgeAzure, URL: server.URL, Token: "secret", AuthKind: AuthPAT}, server.URL, "org")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAzureHeadTreeBlobAndSnapshotPinCommit(t *testing.T) {
	...
	ref := RepositoryRef{Forge: ForgeAzure, Project: "project-id", RemoteID: "repo-id", Name: "repo"}
...
func TestAzureApplyCommitExactConditionalJSON(t *testing.T) {
	...
	result, err := forge.ApplyCommit(context.Background(), ApplyCommitRequest{
		Repository: RepositoryRef{Forge: ForgeAzure, Project: "project-id", RemoteID: "repo-id"}, Branch: "main", ExpectedHead: "base", Message: "message",
...
func TestAzureInitialAndNewBranchOldObjectIDs(t *testing.T) {
	...
	defer server.Close()
	forge, _ := newAzureForgeWithAPI(profile{Provider: ForgeAzure, URL: server.URL, Token: "secret", AuthKind: AuthBearer}, server.URL, "org")
	ref := RepositoryRef{Forge: ForgeAzure, Project: "project-id", RemoteID: "repo-id"}
	if _, err := forge.ApplyCommit(context.Background(), ApplyCommitRequest{Repository: ref, Branch: "main", Message: "initial", Changes: []RemoteChange{{Operation: "create", Path: "README.md"}}}); err != nil {
		t.Fatal(err)
	}
	if _, err := forge.ApplyCommit(context.Background(), ApplyCommitRequest{Repository: ref, Branch: "main", NewBranch: "feature", ExpectedHead: "base", Message: "branch", Changes: []RemoteChange{{Operation: "create", Path: "feature.txt"}}}); err != nil {
		t.Fatal(err)
	}
...
func TestAzureRedirectDoesNotLeakBearer(t *testing.T) {
	...
	defer source.Close()
	forge, _ := newAzureForgeWithAPI(profile{Provider: ForgeAzure, URL: source.URL, Token: "secret", AuthKind: AuthBearer}, source.URL, "org")
