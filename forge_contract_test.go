...
func TestForgeContractValidatedWriterRequestInvariants(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			writer, err := forgeWriter(remote, false)
			if err != nil {
				t.Fatal(err)
			}
			request := valid
	...
	remote := &contractForge{kind: ForgeGitea, capabilities: ForgeCapabilities{Push: true}, result: ApplyCommitResult{CommitID: "next", ConditionalRef: true}}
	writer, _ := forgeWriter(remote, false)
...
func TestForgeContractWriterCapabilityGatesAndErrorIdentity(t *testing.T) {
	...
	if _, err := forgeWriter(disabled, false); !errors.Is(err, ErrUnsupported) || disabled.calls != 0 {
	...
	if _, err := forgeWriter(noBranch, true); !errors.Is(err, ErrUnsupported) {
	...
	writer, _ := forgeWriter(failing, false)
