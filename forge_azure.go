...
func newAzureForge(p profile) (*azureForge, error) {
...
func newAzureForgeWithAPI(p profile, apiBase, organization string) (*azureForge, error) {
	...
	return &azureForge{
		...
		requester: newHTTPRequester(p, apiBase, auth, make(http.Header)),
	}, nil
}
...
func (a *azureForge) ResolveRepository(ctx context.Context, value string) (RepositoryRef, RepositoryInfo, error) {
	...
	if !strings.EqualFold(organization, a.organization) {
		return RepositoryRef{}, RepositoryInfo{}, fmt.Errorf("azure repository organization %q does not match profile organization %q", organization, a.organization)
	}
	...
	if response.ID == "" || response.Name == "" || response.Project.ID == "" || response.Project.Name == "" {
		return RepositoryRef{}, RepositoryInfo{}, errors.New("azure devops returned incomplete repository identity")
	}
	canonical := organization + "/" + response.Project.Name + "/" + response.Name
	ref := RepositoryRef{
		Forge: ForgeAzure, Server: a.server, Namespace: organization, Project: response.Project.ID,
		Name: response.Name, RemoteID: response.ID, Canonical: canonical,
	}
	branch := strings.TrimPrefix(response.DefaultBranch, "refs/heads/")
	return ref, RepositoryInfo{DefaultBranch: branch, Empty: response.DefaultBranch == ""}, nil
}

func (a *azureForge) Head(ctx context.Context, ref RepositoryRef, branch string) (string, error) {
...
func (a *azureForge) Tree(ctx context.Context, ref RepositoryRef, commit string) (map[string]RemoteFile, error) {
	...
	for _, item := range response.Value {
		...
		cleaned, err := validateRemotePath(item.Path)
		if err != nil {
			return nil, err
		}
		if item.ObjectID == "" {
		...
		result[cleaned] = RemoteFile{BlobID: azureBlobID(commit, cleaned), Mode: 0o100644, Size: item.ContentLength}
	}
	return result, nil
}

func (a *azureForge) Blob(ctx context.Context, ref RepositoryRef, file RemoteFile) ([]byte, error) {
...
func (a *azureForge) CommitDetails(ctx context.Context, ref RepositoryRef, commit string) (RemoteCommit, error) {
	...
	result := RemoteCommit{ID: response.CommitID, Message: response.Comment, ParentIDs: append([]string(nil), response.Parents...)}
	for skip := 0; ; skip += 1000 {
		...
		for _, changed := range page.Value {
			cleaned, err := validateRemotePath(changed.Item.Path)
			if err != nil {
				return RemoteCommit{}, err
			}
...
func (a *azureForge) ApplyCommit(ctx context.Context, request ApplyCommitRequest) (ApplyCommitResult, error) {
	...
	if len(response.Commits) != 1 || len(response.RefUpdates) != 1 {
		return ApplyCommitResult{}, errors.New("azure devops returned a malformed push response")
	}
	...
	if commit.CommitID == "" || update.Name != azureFullRef(target) || update.NewObjectID != commit.CommitID {
		return ApplyCommitResult{}, errors.New("azure devops push response did not confirm the requested ref update")
	}
	if request.NewBranch == "" && update.OldObjectID != "" && update.OldObjectID != oldObjectID {
		return ApplyCommitResult{}, errors.New("azure devops push response reported an unexpected previous head")
	}
	if request.NewBranch != "" && update.OldObjectID != "" && update.OldObjectID != azureZeroOID && update.OldObjectID != oldObjectID {
		return ApplyCommitResult{}, errors.New("azure devops new-branch response reported an unexpected previous head")
	}
	result := ApplyCommitResult{CommitID: commit.CommitID, ParentIDs: append([]string(nil), commit.Parents...), ConditionalRef: true}
	if request.ExpectedHead != "" && len(result.ParentIDs) > 0 && result.ParentIDs[0] != request.ExpectedHead {
		return ApplyCommitResult{}, errors.New("azure devops created the commit from an unexpected parent")
	}
	return result, nil
}

func azureRepoAPIPath(ref RepositoryRef) string {
...
func normalizeAzureServer(raw string) (string, string, error) {
	server, err := normalizeServerURL(raw)
	if err != nil {
		return "", "", err
	}
